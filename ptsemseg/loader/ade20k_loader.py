...
class ADE20KLoader(data.Dataset):
    ...
    def transform(self, img, lbl):
        #img = m.imresize(img, (self.img_size[0], self.img_size[1]))  # uint8 with RGB mode return ndarray
        ...
        lbl = np.resize(lbl, (self.img_size[0], self.img_size[1]))
        #lbl = m.imresize(lbl, (self.img_size[0], self.img_size[1]), "nearest", mode="F")
        lbl = lbl.astype(int)


        # GASPAR - CHANGE LATER TO 2 CLASSES ONLY.
        # This is done because there are classes > 150 and (-1) which will trigger the 
        lbl_copy = []
        for i,lbl_sub in enumerate(lbl):
            lbl_sub_copy=[]
            for item in lbl_sub:
                if item < 30:
                    lbl_sub_copy.append(0)
                else:
                    lbl_sub_copy.append(1)
            lbl_copy.append(lbl_sub_copy)
        #------------------------------------------------------------------------------

        lbl_np = np.asarray(lbl)
        lbl_copy_np = np.asarray(lbl_copy)
        #print(lbl_np.shape)
        #print(lbl_copy_np.shape)
        #time.sleep(0.5)
        #print("numero classes:  " + str(np.unique(lbl)))
        #assert np.all(classes == np.unique(lbl))

        #print(lbl_copy_np)
        #print(np.unique(lbl_copy_np))

        img = torch.from_numpy(img).float()
        lbl_copy_np = torch.from_numpy(lbl_copy_np).long()
        return img, lbl_copy_np
...
# Como estamos a importar, não vai aparecer.
